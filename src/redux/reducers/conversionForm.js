    ...
    case UPDATE:
      if (action.payload.which === 'from') {
        return {
          ...
          to: calculateAmount(action.payload.exchangeRates, action.payload.value, state.to),
        };
      } else if (action.payload.which === 'to') {
        return {
          ...state,
          from: calculateAmount(action.payload.exchangeRates, action.payload.value, state.from),
